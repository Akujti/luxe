...
<div class="container-fluid">
    ...
    <div class="row justify-content-center">
        <div class="col-12 col-lg-7 col-xl-5 mb-4">
            <div class="w-100 p-1" id="el">
                <table class="w-100">
                    <tbody>
                        ...
                        <tr>
                            <th>Porjected Sales Price</th>
                            <td class="input"><input type="text" id="b5" onchange="calculate()"></td>
                        </tr>
                        <tr>
                            <th>Interest Rate(if cash put 0)</th>
            ...
            <button class="btn-luxe w-100 my-4" onclick="generatePDF(166, 170)">Generate</button>
        </div>
    </div>
</div>
...
<script>
    function calculate() {
        ...
        closing_costs_purchase.html("$" + (isNaN(closing_costs_purchase_result) ? '0.00' : closing_costs_purchase_result.toFixed(2)));

        // Closing Costs Rate calculator
        var closing_costs_sale = $('#closing-costs-sale');

        var closing_costs_sale_result = parseFloat(b5 * (b8 / 100));
        closing_costs_sale.html("$" + (isNaN(closing_costs_sale_result) ? '0.00' : closing_costs_sale_result.toFixed(2)));
        ...
        interest_only_mortgage_payments.html("$" + (isNaN(interest_only_mortgage_payments_result) ? '0.00' : interest_only_mortgage_payments_result.toFixed(2)));

        
        // Profit calculator
        var profit = $('#profit');
        
        var profit_result = parseFloat((((b5 - closing_costs_sale_result) - b4) - b2) - interest_only_mortgage_payments_result);
        ...
        var roi_result = percentage(profit_result.toFixed(2), (b2 * (1 - (b7 / 100)) + b4 + closing_costs_purchase_result));
